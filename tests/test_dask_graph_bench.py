...
def _make_dask_setup(src_size, chunk_size, dst_size=None):
    ...
    dst_transform_affine, dst_width, dst_height = (
...
@pytest.mark.parametrize("src_size,chunk_size", [
    (256, 64),
    (1024, 256),
    (4096, 512),
    (16384, 512),
...
def test_graph_build(benchmark, src_size, chunk_size):
    ...
    src_dask, src_geobox, dst_geobox, dst_chunks = _make_dask_setup(
        src_size, chunk_size
    )
    ...
    def build_graph():
        return reproject(
            ...
            resampling="nearest", dst_chunks=dst_chunks,
        )
